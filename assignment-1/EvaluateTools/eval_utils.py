...
def run_eval(model, dataset, eval_file, num_batches, batch_size,
    ...
    for Cwid, Ccid, Qwid, Qcid, y1, y2, ids in tqdm(
        ...
        # *FIX-II-008
        # *change: 'yp1 = torch.argmax(p1, dim=0); yp2 = torch.argmax(p2, dim=0)'
        # *rationale: span prediction must choose the best position within each sequence, so argmax should run over the sequence-length axis, not the batch axis
